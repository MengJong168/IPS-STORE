...
def load_transactions():
    try:
        response = requests.get(f'{DATA_STORE_URL}/transactions?store=ipsstore', timeout=5)
...
def save_transactions(transactions):
    try:
        response = requests.post(f'{DATA_STORE_URL}/transactions?store=ipsstore', 
...
def generate_qr():
    try:
        ...
        qr_data = khqr.create_qr(
            bank_account='lenin_chea1@aclb',
            merchant_name='IPS Store',
...
def update_package():
    try:
        ...
        # Update via API
        response = requests.post(f'{DATA_STORE_URL}/packages/update?store=ipsstore', 
                               json={
                                   'game_type': game_type,
